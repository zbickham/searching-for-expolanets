...
#    inputfile = pd.read_csv(r'koi_candidates.csv',nrows=10,usecols = ["kepid","kepoi_name","kepler_name","koi_disposition","koi_pdisposition","koi_score"])
    ...
    csvpath = r'koi_candidates.csv'
    with open(csvpath, encoding='utf-8') as inputfile:
        csvReader = csv.DictReader(inputfile)
        global data
        for row in csvReader:
            if len(jsonarray) == 10:
                break
            key = f"{dict(row)['kepid']}-{dict(row)['kepoi_name']}"
            # -{dict(row)['kepler_name']-{dict(row)['koi_disposition']-{dict(row)['koi_pdisposition']-{dict(row)['koi_score']}"
    ...
    newdata = data['kepid']['kepoi_name']['kepler_name']['koi_disposition']['koi_pdisposition']['koi_score']
    print(newdata)
